<script lang="ts">
  ...
  onMount(async () => {
    try {
      ...
      if (token && token.length > 0 && IsValidToken(token) && !IsTokenExpired(token)) {
        ID = getID(token);
...
<div class="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 flex items-center justify-center py-8">
  <div class="w-full max-w-lg bg-gray-800/80 p-6 rounded-2xl shadow-lg border border-gray-700/50">
    <h2 class="text-2xl font-bold text-white mb-6 text-center">Add New Notice</h2>
    <p class="text-gray-400 text-center mb-6">Added by {name}</p>
    <form on:submit|preventDefault={handleSubmit} class="space-y-6">
      <!-- Title -->
      <div>
        <label for="title" class="block text-gray-400 font-medium mb-2">Notice Title</label>
        <input
          type="text"
      ...
      <!-- Date -->
      <div>
        <label for="date" class="block text-gray-400 font-medium mb-2">Date</label>
        <input
          type="date"
          class="w-full px-4 py-3 bg-gray-700/50 text-white rounded-xl border border-gray-600 focus:ring-2 focus:ring-blue-500 focus:outline-none"
          bind:value={date}
      ...
      <!-- Content -->
      <div>
        <label for="content" class="block text-gray-400 font-medium mb-2">Content</label>
        <textarea
          rows="4"
      ...
      <!-- Year -->
      <div>
        <label for="year" class="block text-gray-400 font-medium mb-2">Year</label>
        <select
          class="w-full px-4 py-3 bg-gray-700/50 text-white rounded-xl border border-gray-600 focus:ring-2 focus:ring-blue-500 focus:outline-none"
          bind:value={year}
        >
      ...
      <!-- File Upload -->
      <div>
        <label for="file" class="block text-gray-400 font-medium mb-2">Upload Files</label>
        <input
          type="file"
          multiple
      ...
      {#if loading}
        <div class="flex justify-center items-center">
          <div class="border-t-4 border-blue-500 w-8 h-8 mx-auto animate-spin rounded-full"></div>
